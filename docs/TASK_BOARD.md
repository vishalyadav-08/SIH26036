...
All tasks use the repository-confirmed Python/Django/DRF backend and Next.js/React frontend. `BACKLOG` means not started; `READY` means dependencies and contracts are available; `BLOCKED` requires evidence and an owner. No task may instruct work in an absent project path.

## Task records

### DOC-001 — Documentation architecture migration

- **Area/Owner/AI agent:** DOCS / Project Lead / doc-agent
- **Target client:** DOCS
- **Objective:** Align active docs to Django + DRF, Next.js/React, PWA fallback, and conditional Flutter strategy.
- **Dependencies:** Repository inspection.
- **Allowed files:** `docs/**`
- **Forbidden files:** `backend/**`, `frontend/**`, database/infra source.
...
- **Acceptance criteria:** All active docs agree; archive is historical; consistency report has no conflicts.
- **Tests:** Obsolete-term search and cross-document review.
- **Verification command:** `rg -n -i --glob "*.md" --glob "!docs/archive/**" "Java|Spring Boot|Maven|JPA|Hibernate|Flyway|services/api|apps/web" docs`
- **Expected output:** Only classified historical/exclusion/migration references in active root docs; no accidental implementation instruction.
- **Status:** READY
...
- **Forbidden files:** `frontend/**`, `docs/archive/**`, unapproved schema changes.
...
- **Forbidden files:** `backend/**`, field persistence contract, private key material.
...
### FIELD-001 — React PWA testing path

- **Area/Owner/AI agent:** FIELD / Field Team / coding-agent
- **Target client:** FIELD_PWA
- **Objective:** Complete the current browser field flow for cached inspection, checklist, readings, evidence, review, and visible sync state.
...
- **Forbidden files:** Backend authorization, Flutter code, private keys, new business API semantics.
- **API dependency:** Assigned inspections, evidence, decision, and `/api/v1/sync`.
...
- **Target client:** BACKEND
- **Objective:** Implement `/api/v1/sync` and server SyncRecord behavior.
...
- **Tests:** Django integration/security tests and PWA replay tests.
...
- **Target client:** CROSS_CUTTING
- **Objective:** Validate, store, and link evidence metadata and binary artifacts.
...
- **Tests:** Django integration/security and PWA evidence tests.
...
- **Forbidden files:** Frontend/private key material, changed primitives, legal-signature claims.
...
### FLUTTER-001 — Flutter readiness evaluation

- **Area/Owner/AI agent:** FIELD / Field Team + Project Lead / evaluation-agent
- **Target client:** FLUTTER
- **Objective:** Decide whether Flutter is ready before the internal hackathon without changing shared contracts.
- **Dependencies:** FIELD-001, OFF-001, ADR-022.
- **Allowed files:** `docs/**`, approved evaluation notes; a future Flutter directory only after approval.
- **Forbidden files:** Unapproved Flutter packages, backend client-specific endpoints, duplicated domain logic.
- **API dependency:** Same `/api/v1` contract as PWA.
- **Data dependency:** Same entities, states, operation envelope, and versions.
- **Security requirements:** Native secure storage and device permissions require review; no private keys.
- **Acceptance criteria:** Readiness checklist records build, lifecycle, offline persistence, sync, camera/location, and test evidence.
- **Tests:** Evaluation smoke/integration evidence.
- **Verification command:** `rg -n "Flutter|FLUTTER-001|readiness" docs`
- **Expected output:** Decision is recorded as ready or not ready with evidence.
- **Status:** BACKLOG / CONDITIONAL

### FLUTTER-002 — Native field client if approved

- **Area/Owner/AI agent:** FIELD / Field Team / coding-agent
- **Target client:** FLUTTER
- **Objective:** Implement the approved native field client against common contracts.
- **Dependencies:** FLUTTER-001 = READY, API-001, OFF-001, security ADR for packages/storage.
- **Allowed files:** Approved Flutter project path and tests only.
- **Forbidden files:** Backend contract changes to accommodate Flutter; PWA removal; private keys.
- **API dependency:** Same endpoints and idempotent sync semantics as PWA.
- **Data dependency:** Same inspection/evidence/sync entities and local states.
- **Security requirements:** Secure local handling, permission denial, logout/re-auth, no local authorization authority.
- **Acceptance criteria:** Mobile lifecycle/offline/sync/camera/location integration passes; PWA remains fallback.
- **Tests:** Dart unit/widget/integration tests, shared API contract tests.
- **Verification command:** `flutter test` (only after a Flutter project and toolchain are approved)
- **Expected output:** Flutter test suite passes; command is not valid before approval.
- **Status:** CONDITIONAL
...
- **Target client:** QA
- **Objective:** Verify the complete client-independent workflow and security gates.
- **Dependencies:** API-001, AUTH-001, FIELD-001, OFF-001, CRYPTO-001, PUB-001.
...
- **Acceptance criteria:** Critical tests cover Web, PWA, and Flutter conditionally; no critical/high finding remains unaccepted.
- **Tests:** Django tests, Vitest/Testing Library, Playwright when configured, security/accessibility/performance checks.
- **Verification command:** `cd backend; python manage.py test; cd ../frontend; pnpm exec vitest run; pnpm lint; pnpm build`
