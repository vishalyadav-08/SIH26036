...
- **QR:** Segno-generated lookup URL; discovery only, not a security proof.
...
7. Generate a QR using Segno containing only `https://<frontend-domain>/verify/<certificateNo>`.
...
The private key is backend-only, never committed, logged, returned by API, stored in the Flutter app, React PWA, React Web, or placed in MinIO as ordinary application data. Prototype key material is injected through protected environment/secret configuration and labelled non-production. Public key/reference may be exposed for verification. Production needs a managed key store/HSM, rotation/versioning, access separation, backup/recovery, revocation, and ceremony policy.
...
The QR is an encoded URL for discovery. A scanner or user opens `/verify/:certNo`; the page calls only `GET /api/v1/certificates/verify?certNo={certificateNo}`. Security comes from backend lookup, status, hash recomputation, and signature verification — not from the QR image or an untrusted client calculation. Flutter and React PWA clients must never calculate certificate trust locally.
