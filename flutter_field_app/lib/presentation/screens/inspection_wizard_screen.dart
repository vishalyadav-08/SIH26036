...
class _InspectionWizardScreenState extends ConsumerState<InspectionWizardScreen> {
  ...
  // Step 2: Readings State
  final TextEditingController _load5kgController = TextEditingController(text: '5.000');
  final TextEditingController _load10kgController = TextEditingController(text: '10.055');
  ...
  final TextEditingController _notesController = TextEditingController(text: 'Standard verification completed under ambient field conditions.');

  @override
  void dispose() {
    _pageController.dispose();
    _load5kgController.dispose();
    _load10kgController.dispose();
  ...
  Future<void> _pickImage() async {
    try {
      ...
      setState(() {
        _capturedImages.add('https://images.unsplash.com/photo-1581092335397-9583fe92d232?auto=format&fit=crop&q=80&w=600');
      });
    }
  }
  ...
  // --- Step 2: Measurements UI (Matching Measurement/code.html) ---
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Card 1: 5kg Load Test (Normal State)
            Container(
              padding: const EdgeInsets.all(AppTheme.standard),
              decoration: BoxDecoration(
                color: AppTheme.surfaceContainerLowest,
                  ...
                  Row(
                    ...
                    children: [
                      Column(
                        ...
                        children: [
                          Text(
                            l10n?.fiveKgLoadTest ?? '5kg Load Test',
                            style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15),
                          ),
                          const SizedBox(height: 2),
                          Row(
                            children: [
                              const Icon(Icons.error_outline, size: 14, color: AppTheme.onSurfaceVariant),
                      ...
                      Container(
                        ...
                        child: const Text('Ref: 5.000 kg', style: TextStyle(fontSize: 12, color: AppTheme.onSurfaceVariant)),
                      ),
                    ],
                  ),
                  const SizedBox(height: 16),
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(l10n?.indicatedKg ?? 'Indicated (kg)', style: const TextStyle(fontSize: 12, color: AppTheme.onSurfaceVariant)),
                            const SizedBox(height: 4),
                            SizedBox(
                              height: 42,
                              child: TextField(
                                controller: _load5kgController,
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(l10n?.errorLabel ?? 'Error', style: const TextStyle(fontSize: 12, color: AppTheme.onSurfaceVariant)),
                            const SizedBox(height: 4),
                            Container(
                              height: 42,
                              ...
                              child: const Text('0.000 kg', style: TextStyle(fontSize: 13, color: AppTheme.onSurfaceVariant)),
                            ),
                          ],
                        ),
                      ),
            ...
            // Card 2: 10kg Load Test (Error Warning State Matching Design HTML)
            Container(
              padding: const EdgeInsets.all(AppTheme.standard),
              decoration: BoxDecoration(
                color: AppTheme.surfaceContainerLowest,
                  ...
                  Row(
                    ...
                    children: [
                      Column(
                        ...
                        children: [
                          Text(
                            l10n?.tenKgLoadTest ?? '10kg Load Test',
                            style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15),
                          ),
                          const SizedBox(height: 2),
                          Row(
                      ...
                      Container(
                        ...
                        child: const Text('Ref: 10.000 kg', style: TextStyle(fontSize: 12, color: AppTheme.onSurfaceVariant)),
                      ),
                    ],
                  ),
                  const SizedBox(height: 16),
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(l10n?.indicatedKg ?? 'Indicated (kg)', style: const TextStyle(fontSize: 12, color: AppTheme.error, fontWeight: FontWeight.bold)),
                            const SizedBox(height: 4),
                            SizedBox(
                              height: 42,
                              child: TextField(
                                controller: _load10kgController,
                                keyboardType: const TextInputType.numberWithOptions(decimal: true),
                                decoration: InputDecoration(
                                  filled: true,
                                  fillColor: AppTheme.errorContainer,
                                  contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 0),
                                  border: OutlineInputBorder(
                                    borderRadius: BorderRadius.circular(AppTheme.radiusMd),
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(l10n?.errorLabel ?? 'Error', style: const TextStyle(fontSize: 12, color: AppTheme.error, fontWeight: FontWeight.bold)),
                            const SizedBox(height: 4),
                            Container(
                              height: 42,
                              padding: const EdgeInsets.symmetric(horizontal: 12),
                              ...
                              child: const Text(
                                '+0.055 kg',
                                style: TextStyle(fontSize: 13, color: AppTheme.onErrorContainer, fontWeight: FontWeight.bold),
                              ),
                            ),
                          ],
                        ),
            ...
            InkWell(
              onTap: () {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Custom load test added.')),
                );
              },
              borderRadius: BorderRadius.circular(AppTheme.radiusLg),
              child: Container(
        ...
        child: Column(
          ...
          children: [
                ...
                Expanded(
                  child: SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      onPressed: _pickImage,
                      icon: const Icon(Icons.upload_file, size: 20, color: AppTheme.onSurface),
                      label: Text(
            ...
            ...List.generate(_capturedImages.length, (idx) {
              final imgUrl = _capturedImages[idx];
              return Container(
                ...
                return GridView.count(
                  ...
                  children: [
                    ...
                    _buildReviewCard(
                      ...
                      subtitle: '2/2 ${isHi ? 'दर्ज की गई' : 'Recorded'}',
                      onEdit: () => _goToStep(1),
                    ),
